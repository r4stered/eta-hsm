...
TEST(TimerTest, StaticTimerBank)
{
    ...
    // The bank's per-group array is sized by enum_count<State>() == 4 (eNone,
    // eRed, eGreen, eBlue); indexing by State enumerator must stay in bounds.
    ...
    // SCENARIO("Same event associated with different states handled independently")
    bank.addTimer(Event::eOne, State::eRed, epoch + std::chrono::seconds(50));
    bank.addTimer(Event::eOne, State::eBlue, epoch + std::chrono::seconds(100));
